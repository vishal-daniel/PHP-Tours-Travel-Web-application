...
<body>
	...
	if (isset($_POST["sbmt"])) {
		$cn = makeconnection();
		$s = "update tours set Tour_name='" . $_POST["t2"] . "' where Tour_id='" . $_POST["t1"] . "'";
		mysqli_query($cn, $s);
		mysqli_close($cn);
		...
		<div class="col-sm-9">
			...
			<form method="post">
				<table border="0" width="500px" height="300px" align="center" class="tableshadow">
					<tr>
						<td colspan="2" class="toptd">Update Tour</td>
					</tr>
					<tr>
						<td class="lefttxt">Selected Tour Name</td>
						<td><select name="t1" required />
							...
							if (isset($_POST["show"])) {
								$cn = makeconnection();
								$s = "select * from Tours where Tour_id='" . $_POST["t1"] . "'";
								$result = mysqli_query($cn, $s);
								$r = mysqli_num_rows($result);
								//echo $r;
